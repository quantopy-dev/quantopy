...
class TestGenerator:
    ...
    def test_returns_lognormal(self) -> None:
        # Test single mu and sigma
        mu, sigma = 0, 0.1  # mean and standard deviation
        ...
        # Test list of mu and sigma
        mu_list = [0.1, 0.05]  # mean
        sigma_list = [0.1, 0.05]  # standard deviation
        rdf = qp.random.generator.returns(mu_list, sigma_list, 10000)
        assert type(rdf) is qp.ReturnDataFrame
        ...
        assert_almost_equal(log_rdf.iloc[:, 1].mean(), mu_list[1], decimal=2)
        assert_almost_equal(log_rdf.iloc[:, 1].std(), sigma_list[1], decimal=2)
    ...
    def test_prices_lognormal(self) -> None:
        # Test single mu and sigma
        mu, sigma = 0, 0.1  # mean and standard deviation
        initial_price = 10
        ps = qp.random.generator.prices(initial_price, mu, sigma, 10000, "lognormal")
        rs = qp.ReturnSeries.from_price(ps)
        assert type(ps) is pd.Series
        ...
        # Test list of mu and sigma
        mu_list = [0.3, 0.05]  # mean
        sigma_list = [0.3, 0.05]  # standard deviation
        initial_price_list = [1, 2]
        pdf = qp.random.generator.prices(
            initial_price_list, mu_list, sigma_list, 2000, "lognormal"
        )
        rdf = qp.ReturnDataFrame.from_price(pdf)
        assert type(pdf) is pd.DataFrame
